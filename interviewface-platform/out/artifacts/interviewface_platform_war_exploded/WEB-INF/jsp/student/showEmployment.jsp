			...
			<div class="col-md-10">
				<div class="panel panel-default">
				    ...
				    <table class="table table-bordered">
					        ...
					        <tbody>
							<c:forEach  items="${employmentList}" var="item" varStatus="status">
								<tr>
...
<%--										<button class="btn btn-default btn-xs btn-info" onClick="location.href='/school/editSchool?id=${item.schoolId}'">修改/审核</button>--%>
                                        <input type="file" style="display:inline-block;float: left;" id="file${status.index}" name="file">${item.file}
										<button id= "upload-btn" class="btn btn-primary" style="display:inline-block;" onclick="postPDF(${item.employmentId},${item.employerId},file${status.index})">投递</button>
										<!--弹出框-->
									</td>
								</tr>
							</c:forEach>
	...
	<script type="text/javascript">
        ...
        function postPDF(employmentId,itemId,filePath) {
            var str = '';
            ...
            if(str === ''){
                ...
                $.ajaxFileUpload({
                    url:'/student/ajaxUploadPdf?employmentId='+employmentId+'&id='+itemId,
                    type:'POST',
                    fileElementId :filePath.id,
                    success:function (messageVo) {
                        var json = $.parseJSON(messageVo.body.innerText);
                        alert(json.message);
